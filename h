...
export default async function handler(req, res) {
  try {
    ...
    const [{ data: interactions }, { data: prefs }, { data: articles }] = await Promise.all([
      supabase.from('user_interactions').select('*, articles(title)').eq('user_id', user_id).order('timestamp', { ascending: false }).limit(50),
    ...
    const prompt = `Rank these articles for a user.
Liked articles: ${liked}
Disliked articles: ${disliked}
Favourite topics: ${topicPrefs}
Favourite sources: ${sourcePrefs}
Articles:
${articles.map(a => JSON.stringify({ id: a.id, title: a.title, source: a.source })).join('\n')}
